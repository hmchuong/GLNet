...
class Evaluator(object):
    ...
    def eval_test(self, sample, model, global_fixed):
        with torch.no_grad():
            ...
            labels =[RGB_mapping_to_class(np.array(label)) for label in sample['label']]
            if not self.test:
                self.metrics_global.update(labels, predictions_global)

            if self.mode == 2 or self.mode == 3:
                # patch predictions ###########################
                predictions_local = [ score.argmax(1)[0] for score in scores_local ]
                if not self.test:
                    self.metrics_local.update(labels, predictions_local)
                ###################################################
                # combined/ensemble predictions ###########################
