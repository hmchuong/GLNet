...
from dataset.idrid import IDRID, classToRGB
...
dataset_train = IDRID(data_path, train_df, label=True, transform=True)
...
dataset_val = IDRID(data_path, val_df, label=True)
...
dataset_test = IDRID(data_path, val_df, label=False)
...
for epoch in range(num_epochs):
    ...
    if epoch % 1 == 0:
        with torch.no_grad():
            ...
            for i_batch, sample_batched in enumerate(tbar):
                ...
                if test:
                    ...
                    for i in range(len(images)):
                        if mode == 1:
                            transforms.functional.to_pil_image(classToRGB(predictions_global[i])).save(os.path.join(prediction_dir, sample_batched['id'][i] + "_mask.png"))
                        else:
                            transforms.functional.to_pil_image(classToRGB(predictions[i])).save(os.path.join(prediction_dir, sample_batched['id'][i] + "_mask.png"))
            ...
            if not (test or evaluation): torch.save(model.state_dict(), os.path.join(args.model_path, task_name + ".pth"))
