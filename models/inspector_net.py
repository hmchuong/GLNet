...
class ResnetFPN(nn.Module):
    def __init__(self, numClass):
        ...
        #self.classify = nn.ConvTranspose2d(128*4, numClass, (5,5), (4,4))
...
class ResnetFPNLocal(nn.Module):
    ...
    def forward(self, input):
        ...
        c2, c3, c4, c5 = self.resnet_backbone(image)
        # Top-down
        p5 = self.toplayer(torch.cat([c5] + [self.bn_top(F.interpolate(c5_ext, size=c5.size()[2:], **self._up_kwargs))], dim=1))
        p4 = self._upsample_add(p5, self.latlayer1(torch.cat([c4] + [self.bn_latlayer1(F.interpolate(c4_ext, size=c4.size()[2:], **self._up_kwargs))], dim=1)))
        p3 = self._upsample_add(p4, self.latlayer2(torch.cat([c3] + [self.bn_latlayer2(F.interpolate(c3_ext, size=c3.size()[2:], **self._up_kwargs))], dim=1)))
        p2 = self._upsample_add(p3, self.latlayer3(torch.cat([c2] + [self.bn_latlayer3(F.interpolate(c2_ext, size=c2.size()[2:], **self._up_kwargs))], dim=1)))
        ps0 = [p5, p4, p3, p2]
        
        # Smooth
        p5 = self.smooth1_1(torch.cat([p5] + [self.bn_smooth1_1(F.interpolate(ps0_ext[0], size=p5.size()[2:], **self._up_kwargs))], dim=1))
        p4 = self.smooth2_1(torch.cat([p4] + [self.bn_smooth2_1(F.interpolate(ps0_ext[1], size=p4.size()[2:], **self._up_kwargs))], dim=1))
        p3 = self.smooth3_1(torch.cat([p3] + [self.bn_smooth3_1(F.interpolate(ps0_ext[2], size=p3.size()[2:], **self._up_kwargs))], dim=1))
        p2 = self.smooth4_1(torch.cat([p2] + [self.bn_smooth4_1(F.interpolate(ps0_ext[3], size=p2.size()[2:], **self._up_kwargs))], dim=1))
        ps1 = [p5, p4, p3, p2]
        
        p5 = self.smooth1_2(torch.cat([p5] + [self.bn_smooth1_2(F.interpolate(ps1_ext[0], size=p5.size()[2:], **self._up_kwargs))], dim=1))
        p4 = self.smooth2_2(torch.cat([p4] + [self.bn_smooth2_2(F.interpolate(ps1_ext[1], size=p4.size()[2:], **self._up_kwargs))], dim=1))
        p3 = self.smooth3_2(torch.cat([p3] + [self.bn_smooth3_2(F.interpolate(ps1_ext[2], size=p3.size()[2:], **self._up_kwargs))], dim=1))
        p2 = self.smooth4_2(torch.cat([p2] + [self.bn_smooth4_2(F.interpolate(ps1_ext[3], size=p2.size()[2:], **self._up_kwargs))], dim=1))
        ps2 = [p5, p4, p3, p2]
        
        # Classify
        ps3 = self._concatenate(
                torch.cat([p5] + [self.bn_ps2_0(F.interpolate(ps2_ext[0], size=p5.size()[2:], **self._up_kwargs))], dim=1), 
                torch.cat([p4] + [self.bn_ps2_1(F.interpolate(ps2_ext[1], size=p4.size()[2:], **self._up_kwargs))], dim=1), 
                torch.cat([p3] + [self.bn_ps2_2(F.interpolate(ps2_ext[2], size=p3.size()[2:], **self._up_kwargs))], dim=1), 
                torch.cat([p2] + [self.bn_ps2_3(F.interpolate(ps2_ext[3], size=p2.size()[2:], **self._up_kwargs))], dim=1)
            )
        ps3 = self.smooth(ps3)
        output = self.classify(ps3)
